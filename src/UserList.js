    ...
    const fetchUsers = async () => {
        try {
            ...
            const userPromises = userAddresses.map(async (address) => {
                const userCount = await contract.getUserCount(address);
                const userDetailsPromises = Array.from({ length: userCount }, (_, index) =>
                    contract.getUserDetails(address, index)
                );
            ...
            setUsers(userList.flat()); // Flatten the array of users
        } catch (error) {
